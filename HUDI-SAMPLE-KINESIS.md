...
1. Create Kinesis Data Stream with name data-stream-ingest in a region. The region name needs to be given. 
2. Setup the bucket , role , virtual server id and other parameters. Ensure that the execution role has permission on S3 bucket and log group. 
```shell
...
export HUDI_TABLE_NAME=eks_fargate_hudi_table
...
```
3. Checkout and build kinesis-sql library required for the job. 
```shell
...
```
4. Checkout and build sample project. 
```shell
